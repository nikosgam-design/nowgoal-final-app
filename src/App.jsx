...
const STORAGE_KEY = "nowgoal_saved_predictions_v3";
const SETTINGS_KEY = "nowgoal_web_settings_v3";
...
const DEMO_LIVE_MATCHES = [
  {
    ...
    sport: "football",
    league: "Super League",
    matchDate: "20-04-2026",
    matchTime: "19:30",
    home: "Fenerbahce",
    away: "Besiktas",
    raw: `Super League 20-04-2026 3-2 1-1 7-5
Fenerbahce
VS
Besiktas
74' 1-1 live football premier league win draw lose`,
  },
  {
    ...
    home: "Brighton",
    away: "Newcastle",
    raw: `Premier League 20-04-2026 2-1 1-0 6-4
Brighton
VS
Newcastle
68' 1-0 live football win draw lose`,
  },
  {
    ...
    home: "Lakers",
    away: "Suns",
    raw: `NBA 20-04-2026 118-114 58-55 basketball nba win rate`,
  },
];
...
const CUP_KEYWORDS = [
  "CUP", "COPA", "COUPE", "POKAL", "BEKER", "FA CUP", "LEAGUE CUP",
  "CHAMPIONS", "EUROPA", "CONFERENCE", "SUPER CUP", "WORLD CUP",
  "PLAYOFF", "PLAY-OFF", "FRIENDLY", "INT CF", "ACL", "FIBA"
];
...
function safePct(count, total) {
  return total ? Math.round((count / total) * 100) : 0;
}

function isCup(league = "") {
  const s = league.trim().toUpperCase();
  return CUP_KEYWORDS.some((kw) => s.includes(kw));
}
...
function guessTeams(raw) {
  ...
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].toUpperCase() === "VS" && i > 0 && i < lines.length - 1) {
...
function guessLeague(raw) {
  for (const pat of ["Serie A", "Premier League", "La Liga", "Bundesliga", "Ligue 1", "Euroleague", "NBA", "Basketball Bundesliga"]) {
    if (raw.toLowerCase().includes(pat.toLowerCase())) return pat;
  }
  ...
  for (const line of lines) {
    ...
    if (sport === "football") {
      ...
      rows.push({
        ...
        total_1h: h1 + a1,
        btts: hg > 0 && ag > 0,
        total_c: hc != null && ac != null ? hc + ac : null,
      });
    } else {
...
function footballStats(rows) {
  ...
  if (corners.length) {
    cstats = {
      avg: Number((corners.reduce((a, b) => a + b, 0) / corners.length).toFixed(1)),
      o85: safePct(corners.filter((x) => x > 8.5).length, corners.length),
      o95: safePct(corners.filter((x) => x > 9.5).length, corners.length),
      o105: safePct(corners.filter((x) => x > 10.5).length, corners.length),
    };
  }
  return {
    sample: total,
    ...
    away,
    corners: cstats,
    avg_goals: Number((totals.reduce((a, b) => a + b, 0) / total).toFixed(2)),
    avg_1h: Number((firstHalf.reduce((a, b) => a + b, 0) / total).toFixed(2)),
  };
}
...
function basketballStats(rows) {
  ...
  return {
    ...
    home,
    away,
    avg_total: Number((totals.reduce((a, b) => a + b, 0) / total).toFixed(1)),
    avg_1h: Number((firstHalf.reduce((a, b) => a + b, 0) / total).toFixed(1)),
  };
}
...
function ActionButton({ color, children, icon, onClick }) {
  return (
    <button onClick={onClick} style={{ ...styles.buttonBase, background: color, display: "inline-flex", alignItems: "center", gap: 8 }}>
      {icon}
      {children}
    </button>
  );
...
export default function App() {
  ...
  async function saveSavedWithSync(items) {
    ...
    try {
      ...
      setSyncStatus("Firebase sync failed");
    }
  }
  ...
  async function pasteAndAnalyze() {
    try {
      ...
      alert("Clipboard access was blocked. Paste manually in the box.");
    }
  }
  ...
  async function clearAll() {
    ...
    if (db) {
      try {
        ...
        setSyncStatus("Firebase clear failed");
      }
    }
    setSaved([]);
    saveSavedLocal([]);
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} style={{ ...styles.card, padding: 20 }}>
          <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "space-between", gap: 16, alignItems: "center" }}>
            <div style={{ minWidth: 280, flex: 1 }}>
              <div style={{ fontSize: 34, fontWeight: 800, color: "#38bdf8" }}>Nowgoal Ultimate Final</div>
              <div style={{ color: "#94a3b8", marginTop: 8 }}>Analyzer, Live Auto, Telegram alerts, cloud save, mobile-ready layout, and deploy-ready placeholders.</div>
            ...
            <Panel title="Cloud Save + Telegram Settings">
              <div style={{ display: "grid", gap: 10 }}>
                ...
                <div style={{ fontSize: 12, color: "#64748b" }}>
                  Placeholder mode is ready. After deploy, paste your real Telegram and Firebase values here.
                </div>
              </div>
            </Panel>
            ...
            {mode === "analyzer" ? (
              <Panel title="Paste Raw Match Data">
                <TextAreaField
                  ...
                  placeholder="Paste the Nowgoal match text here..."
                />
              </Panel>
            ) : (
              <Panel title="Live Auto Scanner">
                ...
                <ActionButton color="#7c3aed" icon={<BellRing size={16} />} onClick={runLiveScan}>
                  {liveLoading ? "Scanning..." : "Run Live Scan"}
                </ActionButton>
                <div style={{ display: "grid", gap: 10, marginTop: 12 }}>
                  {liveMatches.length ? liveMatches.map((m) => (
                    <div key={m.id} style={{ border: "1px solid #334155", background: "#020617", borderRadius: 18, padding: 14 }}>
                      ...
                      <div style={{ marginTop: 10, display: "inline-flex", padding: "8px 12px", borderRadius: 999, ...(m.best ? edgeClass(m.best.edge) : edgeClass(0)) }}>
                        {m.best ? `BEST: ${m.best.signal}` : "No signal"}
                      </div>
                    </div>
              ...
              {analyzed?.best ? (
                <div style={{ border: "1px solid rgba(74,222,128,0.35)", background: "rgba(2,6,23,0.8)", borderRadius: 18, padding: 16 }}>
                  ...
                  <div style={{ marginTop: 10, color: "#94a3b8" }}>
                    Top Edge: <span style={{ color: "#86efac", fontWeight: 700 }}>{analyzed.best.edge}%</span>
                  </div>
                </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {mode === "analyzer" ? (
              <Panel
                title="Match Block"
                right={analyzed?.best ? (
                  <div style={{ border: "1px solid rgba(56,189,248,0.25)", background: "rgba(56,189,248,0.10)", color: "#7dd3fc", borderRadius: 999, padding: "8px 12px", fontWeight: 700 }}>
                    TOP EDGE {analyzed.best.edge}%
                  </div>
                ) : null}
              >
                {analyzed ? (
                  <div>
                    <div style={{ color: "#94a3b8", marginBottom: 12 }}>
                      League: {analyzed.league} | Date: {analyzed.matchDate} | Time: {analyzed.matchTime}
                    </div>
                      ...
                      {analyzed.merged.map((item, i) => {
                        ...
                        return (
                          <motion.div
                            ...
                            style={{ borderRadius: 18, padding: 16, ...c }}
                          >
              ...
              <Panel
                title="Live Auto Dashboard"
                right={<div style={{ border: "1px solid rgba(168,85,247,0.25)", background: "rgba(168,85,247,0.10)", color: "#c4b5fd", borderRadius: 999, padding: "8px 12px", fontWeight: 700 }}>{liveMatches.length} live matches</div>}
              >
                {liveMatches.length ? (
                  <div style={{ display: "grid", gap: 12 }}>
                    {liveMatches.map((item, i) => {
                      ...
                      return (
                        <motion.div key={i} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} style={{ borderRadius: 18, padding: 16, ...c }}>
            ...
            <div style={{ display: "grid", gap: 16, gridTemplateColumns: "repeat(auto-fit,minmax(260px,1fr))" }}>
              <Panel title="Value Logic">
                {analyzed?.valueAngles?.length ? (
                  <div style={{ display: "grid", gap: 10 }}>
                    {analyzed.valueAngles.map(([name, edge], i) => (
                      <div key={i} style={{ border: "1px solid rgba(249,115,22,0.3)", background: "rgba(249,115,22,0.10)", borderRadius: 16, padding: 14, color: "#fdba74" }}>
                        {name} — EDGE {edge}%
                      </div>
                    ))}
                  </div>
                ) : <div style={{ color: "#94a3b8" }}>No clear value angle.</div>}
              </Panel>

              <Panel title="Live Signals">
                {analyzed?.liveSignals?.length ? (
                  <div style={{ display: "grid", gap: 10 }}>
                    {analyzed.liveSignals.map(([name, edge, ctx], i) => (
                      <div key={i} style={{ border: "1px solid rgba(244,63,94,0.3)", background: "rgba(244,63,94,0.10)", borderRadius: 16, padding: 14, color: "#fda4af" }}>
                        {name} — {ctx} — EDGE {edge}%
                      </div>
                    ))}
                  </div>
                ) : <div style={{ color: "#94a3b8" }}>No live signal right now.</div>}
              </Panel>
            </div>
                ...
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                  <div style={{ position: "relative", minWidth: 240 }}>
                    ...
                    <input
                      ...
                      placeholder="Search saved matches..."
            ...
            >
              {filteredSaved.length ? (
                <div style={{ display: "grid", gap: 12 }}>
                  {filteredSaved.map((item, i) => (
                    <div key={`${item.match}-${item.matchDate}-${i}`} style={{ border: "1px solid #334155", background: "#020617", borderRadius: 18, padding: 16 }}>
              ...
              ) : <div style={{ color: "#94a3b8" }}>No saved matches yet.</div>}
            </Panel>
          </div>
        </div>
      </div>
